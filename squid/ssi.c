...
load_indexfile(SSIFILE *sfp) {
  ...
  sfp->rpl        = NULL;
  sfp->nfiles     = 0;
  if (! read_i32(sfp->fp, &magic))               {
    status = SSI_ERR_BADMAGIC;
    goto FAILURE;
  }
  if (magic != v20magic && magic != v20swap)     {
    status = SSI_ERR_BADMAGIC;
    goto FAILURE;
  }
  ...
  if ((sfp->filename=malloc(sizeof(char *)    *sfp->nfiles)) == NULL)   {
    status = SSI_ERR_MALLOC;
    goto FAILURE;
  }
  for (i = 0; i < sfp->nfiles; i++) sfp->filename[i] = NULL;
  if ((sfp->fileformat=malloc(sizeof(sqd_uint32)*sfp->nfiles)) == NULL) {
    status = SSI_ERR_MALLOC;
    goto FAILURE;
  }
  if ((sfp->fileflags =malloc(sizeof(sqd_uint32)*sfp->nfiles)) == NULL) {
    status = SSI_ERR_MALLOC;
    goto FAILURE;
  }
  if ((sfp->bpl     =malloc(sizeof(sqd_uint32)*sfp->nfiles)) == NULL)   {
    status = SSI_ERR_MALLOC;
    goto FAILURE;
  }
  if ((sfp->rpl     =malloc(sizeof(sqd_uint32)*sfp->nfiles)) == NULL)   {
    status = SSI_ERR_MALLOC;
    goto FAILURE;
  }
  ...
  for (i = 0; i < sfp->nfiles; i++) {
    ...
    if ((sfp->filename[i] =malloc(sizeof(char)*sfp->flen)) == NULL)        {
      status = SSI_ERR_MALLOC;
      goto FAILURE;
    }
...
binary_search(SSIFILE *sfp, char *key, int klen, SSIOFFSET *base,
  ...
  while (1) {     /* A binary search: */
    ...
    if      (cmp == 0) break;           /* found it!              */
      ...
      free(name);
      return SSI_ERR_NO_SUCH_KEY;
    } else if (cmp < 0)       left  = mid+1; /* it's right of mid     */
    else if (cmp > 0) {
      if (mid == 0) {
        free(name);  /* special case, beware */
        return SSI_ERR_NO_SUCH_KEY;
