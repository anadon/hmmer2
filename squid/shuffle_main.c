...
main(int argc, char **argv) {
  ...
  enum   {    /* shuffling strategy */
    ...
    DO_IID_SAMELEN, DO_IID_FIXEDLEN
  } strategy;
  ...
  while (ReadSeq(dbfp, dbfp->format, &seq, &sqinfo)) {
    ...
    for (i = 0; i < num; i++) {
      ...
      case DO_DPSHUFFLE:
        ...
        break;
      case DO_MARKOV0:
        StrMarkov0(shuff, seq);
        break;
      case DO_MARKOV1:
        StrMarkov1(shuff, seq);
        break;
      case DO_REVERSE:
        StrReverse(shuff, seq);
        break;
      case DO_REGIONAL:
        StrRegionalShuffle(shuff, seq, w);
        break;
      case DO_IID_SAMELEN:
        free(shuff);
        ...
        break;
      default:
        Die("choked on a bad enum; tragic.");
      }
